...
void RunIOSForXSeconds(asio::io_context &IOS, unsigned int seconds)
{
...
void Wait(asio::io_context &IOS, int seconds)
{
	Timer_t timer(IOS);
	timer.expires_from_now(std::chrono::seconds(seconds));
	timer.wait();
...
#define STANDARD_TEST_SETUP()\
	TestSetup();\
	asio::io_context IOS(4); // Max 4 threads
...
{
	asio::io_context IOS(2);

	asio::io_context::work work(IOS); // Just to keep things from stopping..
