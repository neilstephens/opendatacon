...
//	if (mb == NULL)
	{
		std::string msg = Name+": Connect error: 'SNMP stack failed'";
...
//	if (s == -1)
	{
		std::string msg = Name+": Connect error: '" +  "'";
...
template<typename T>
int find_index (const SNMPOidCollection<T>& aCollection, uint16_t index)
{
	//TODO: fixme
	return -1;
}
